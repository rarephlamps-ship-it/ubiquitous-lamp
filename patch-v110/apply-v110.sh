...
cat > 'dist/manifest.webmanifest' <<'LPFH_V110_DIST_1'
{
...
cat > 'dist/public/manifest.webmanifest' <<'LPFH_V110_DIST_2'
{
  "name": "LP/FH Deal & Evidence Cockpit",
  "short_name": "LPFH Cockpit",
  "description": "Offline-first valuation og evidence cockpit",
...
cat > 'dist/src/styles/tokens.css' <<'LPFH_V110_DIST_3'
...
cat > 'dist/src/styles/screens.css' <<'LPFH_V110_DIST_4'
...
cat > 'android/app/build.gradle' <<'LPFH_V110_DIST_5'
...
node - <<'NODE'
const fs=require('fs');const p='dist/BUILD_INFO.json';const j=JSON.parse(fs.readFileSync(p,'utf8'));j.version='1.1.0';j.built_at=new Date().toISOString();fs.writeFileSync(p,JSON.stringify(j,null,2));
NODE
