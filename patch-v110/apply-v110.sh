...
cat > 'package.json' <<'LPFH_V110_1'
{
  "name": "lp-fh-deal-cockpit",
  "version": "1.1.0",
  "type": "module",
  "private": true,
  "scripts": {
    ...
    "serve": "python3 -m http.server 4173 -d dist",
    "verify": "npm test && npm run build && node --check src/app.js",
    "prepare:android": "npm run build && node scripts/prepare-android.mjs"
  }
}
LPFH_V110_1

cat > 'scripts/build.mjs' <<'LPFH_V110_2'
import { rm, mkdir, cp, copyFile, writeFile, readFile } from 'node:fs/promises';
import { join } from 'node:path';
const root=process.cwd(),dist=join(root,'dist');
const pkg=JSON.parse(await readFile(join(root,'package.json'),'utf8'));
await rm(dist,{recursive:true,force:true});await mkdir(dist,{recursive:true});
for(const name of ['index.html','manifest.webmanifest','sw.js'])await copyFile(join(root,name),join(dist,name));
await cp(join(root,'src'),join(dist,'src'),{recursive:true});await cp(join(root,'public'),join(dist,'public'),{recursive:true});
await writeFile(join(dist,'BUILD_INFO.json'),JSON.stringify({app:'LP/FH Deal & Evidence Cockpit',version:pkg.version,built_at:new Date().toISOString(),offline_first:true},null,2));
console.log(`Built dist/ v${pkg.version}`);
LPFH_V110_2

cat > 'android/app/build.gradle' <<'LPFH_V110_3'
...
cat > 'manifest.webmanifest' <<'LPFH_V110_4'
{
...
cat > 'public/manifest.webmanifest' <<'LPFH_V110_5'
{
  "name": "LP/FH Deal & Evidence Cockpit",
  "short_name": "LPFH Cockpit",
  "description": "Offline-first valuation og evidence cockpit",
...
cat > 'src/styles/tokens.css' <<'LPFH_V110_6'
...
cat > 'src/styles/screens.css' <<'LPFH_V110_7'
...
grep -q -- '--gold:#d4b35a' src/styles/tokens.css
grep -q 'linear-gradient(135deg,#16334a' src/styles/screens.css
npm test
npm run build
node --check src/app.js
test "$(node -p "require('./package.json').version")" = '1.1.0'
grep -q '"version": "1.1.0"' dist/BUILD_INFO.json
echo 'LPFH_V110_PATCH_OK'
